...
class ChatSession(BaseModel):
    ...
    platform_type: str = Field(default="AEM")  # "AEM" or "EDS"
...
class ChatStorage:
    ...
    def connect(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.db.chat_sessions.create_index("platform_type")
    ...
    def create_chat_session(self, session_title: str, user_id: Optional[str] = None,
                            model_provider: str = "openai", platform_type: str = "AEM") -> str:
    ...
    def add_message_to_session(self, session_id: str, message: ChatMessage) -> bool:
        ...
        try:
            logger.info(f"💾 [ChatStorage] Adding message to session {session_id}")
            logger.info(f"💾 [ChatStorage] Message type: {message.message_type}, content length: {len(message.content)}")
            ...
            logger.info(f"💾 [ChatStorage] MongoDB update result - matched: {result.matched_count}, modified: {result.modified_count}")
            
            if result.matched_count == 0:
                logger.error(f"❌ [ChatStorage] Session {session_id} NOT FOUND in database!")
            elif result.modified_count == 0:
                logger.warning(f"⚠️  [ChatStorage] Session {session_id} found but NOT modified!")
            else:
                logger.info(f"✅ [ChatStorage] Message successfully added to session {session_id}")
            
            return result.modified_count > 0
        except Exception as e:
            logger.error(f"❌ [ChatStorage] Failed to add message to session {session_id}: {e}", exc_info=True)
            return False
    ...
    def add_component_to_session(self, session_id: str, component: GeneratedComponent) -> bool:
        ...
        try:
            logger.info(f"💾 [ChatStorage] Adding component to session {session_id}")
            logger.info(f"💾 [ChatStorage] Component ID: {component.component_id}, Name: {component.component_name}")
            ...
            if result.matched_count == 0:
                ...
                logger.info(f"✅ [ChatStorage] Component successfully added to session {session_id}")
            
            return result.modified_count > 0
        except Exception as e:
            logger.error(f"❌ [ChatStorage] Failed to add component to session {session_id}: {e}", exc_info=True)
            return False

    def get_user_chat_sessions(self, user_id: Optional[str] = None, limit: int = 20, platform_type: str = "AEM") -> List[ChatSession]:
        """Get chat sessions for a user (or all sessions if user_id is None)"""
        try:
            query = {"is_active": True, "platform_type": platform_type}
            if user_id:
    ...
    def search_chat_sessions(self, search_term: str, user_id: Optional[str] = None, limit: int = 10, platform_type: str = "AEM") -> List[ChatSession]:
